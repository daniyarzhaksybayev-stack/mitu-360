...
const locations = [
  {
    ...
    description: 'Это начальная точка виртуального тура по METU. Здесь пользователь начинает знакомство с университетом, может осмотреть входную зону и перейти к дальнейшим локациям маршрута.',
  ...
  },
  {
    ...
    title: 'Первый корпус',
    type: 'Внешняя локация',
    description: 'Первый корпус является одной из основных учебных локаций университета. Через виртуальный тур абитуриент может заранее увидеть внешний вид корпуса и понять его расположение на территории.',
  ...
  },
  {
    ...
    description: 'Центральная часть кампуса соединяет основные объекты маршрута. Эта зона помогает пользователю лучше ориентироваться между корпусами и выбирать дальнейшее направление движения.',
  ...
  },
  {
    ...
    title: 'Парковка',
    type: 'Внешняя локация',
    description: 'Данная локация показывает подъездную и парковочную зону рядом с университетом. Она помогает посетителям и абитуриентам понять, с какой стороны удобнее подойти к территории.',
  ...
  },
  {
    ...
    type: 'Входная зона',
    description: 'Здесь показан вход во второй корпус университета. Локация помогает заранее ознакомиться с расположением входной зоны и перейти к внутренней части виртуального тура.',
  ...
  },
  {
    ...
    description: 'Холл второго корпуса демонстрирует внутреннее пространство университета. Эта локация показывает, что виртуальный гид может использоваться не только для просмотра территории, но и для знакомства с помещениями учебного заведения.',
...
function createHotspot(hotSpotDiv, args) {
  ...
  button.type = 'button';
  button.className = 'panorama-hotspot-button panorama-hotspot-arrow';
  button.setAttribute('aria-label', label);
  button.innerHTML = `
    <svg class="hotspot-chevron" viewBox="0 0 64 36" aria-hidden="true">
      <polyline points="8,28 32,8 56,28"></polyline>
    </svg>
...
function loadScene(locationId) {
  ...
  if (locationIndicator) locationIndicator.textContent = location.title;
  const prevButton = document.querySelector('#prevBtn');
  const nextButton = document.querySelector('#nextBtn');
  if (prevButton) {
    prevButton.disabled = !location.prev;
    prevButton.setAttribute('aria-disabled', String(!location.prev));
  }
  if (nextButton) {
    nextButton.disabled = !location.next;
    nextButton.setAttribute('aria-disabled', String(!location.next));
  }
...
function scrollToTourTop() {
  requestAnimationFrame(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  });
}

function openLocationFromMap(locationId) {
  closeMap();
...
function changePanoramaZoom(direction) {
  if (!viewer || typeof viewer.getHfov !== 'function' || typeof viewer.setHfov !== 'function') return;
  const currentHfov = Number(viewer.getHfov());
  if (!Number.isFinite(currentHfov)) return;
  const step = 12;
  const minHfov = 35;
  const maxHfov = 120;
  const nextHfov = direction === 'in'
    ? Math.max(minHfov, currentHfov - step)
    : Math.min(maxHfov, currentHfov + step);
  viewer.setHfov(nextHfov, 300);
}

function togglePanoramaFullscreen() {
  const wrapper = document.querySelector('#panoramaWrapper');
  if (!wrapper) return;
  if (document.fullscreenElement) {
    if (typeof document.exitFullscreen === 'function') {
      document.exitFullscreen().catch(error => console.warn('Не удалось выйти из полноэкранного режима', error));
    }
    return;
  }
  if (typeof wrapper.requestFullscreen === 'function') {
    wrapper.requestFullscreen().catch(error => console.warn('Не удалось открыть полноэкранный режим', error));
  }
}
...
function buildMapMarkers(markerContainerId = 'mapMarkers', listContainerId = 'locationList') {
  ...
  locations.forEach(location => {
    ...
    marker.addEventListener('click', () => {
      openLocationFromMap(location.id);
    });
...
function initUI() {
  ...
  safeBind('#zoomOutBtn', () => changePanoramaZoom('out'));
  safeBind('#zoomInBtn', () => changePanoramaZoom('in'));
  safeBind('#fullscreenBtn', togglePanoramaFullscreen);
